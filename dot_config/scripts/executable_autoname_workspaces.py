...
WINDOW_ICONS = {
    ...
    # TODO: Use 'rust' once python FA >= 5.14
    'jetbrains-clion': fa.icons['code'],
    ...
    'xst-256color': fa.icons['terminal'],
...
# also renumbers them in ascending order, each hand controls a monitor
# for example: workspace numbering on two monitors: [1, 2, 3, 4, 5], [6, 7, ..]
def rename_workspaces(i3):
    ws_infos = i3.get_workspaces()
    prev_output = None
    n = 1
    # Sort workspaces by number first so they aren't arbirarily sorted by how
    # the monitors are plugged in
    wspaces = sorted(enumerate(i3.get_tree().workspaces()),
                     key=lambda x: ws_infos[x[0]].num)
    for ws_index, workspace in wspaces:
